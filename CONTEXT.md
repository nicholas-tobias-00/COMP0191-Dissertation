...
- **Next phase:** Forecasting (`05_benchmarking`) — partially-pooled global model carrying livestock+FCO₂+density+lags+mgmt+**gap-filled met+GPP**; Tower 2 split redesign
...
**Replications (R-01–R-03) + CO₂-aug (03b) + feature-eng (F-01, F-02) done; R-04 dropped.** Key arc (Tower 4 short gaps): met-only ≈ −0.13 → +FCO₂ +0.156 → +livestock **+0.256**. Livestock = #1 driver. **F-02: pooling T2+T4+T9 with stocking-density livestock → Tower 9 R² ≈ +0.29 (best in project).** Next steps, in priority order:

1. **Begin forecasting (`05_benchmarking`)** — carry livestock + FCO₂ + **partial-pooled stocking-density** forward (lagged livestock/FCO₂ are legitimate non-co-failed predictors); baselines → RF/XGB → LSTM/TFT, temporal CV. This is the project's novel contribution.
...
---
_Last updated: 2026-06-16 (F-06 complete: Python REddyProc-style pipeline — src/data/reddyproc_pipeline.py (met gap-fill + u* + GPP/Reco partitioning) + F06 notebook + F06_results.md; D-33 added; benchmarks.csv 2740 rows. Finding: proper met gap-fill beats mean-imputation AND GPP helps → Tower 9 +0.335 (new project best); first non-redundant gain since pooling. New best config = partial pool + density + lags + pruned mgmt + gap-filled met + GPP. Next: forecasting.)_
