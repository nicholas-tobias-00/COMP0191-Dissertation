...
1. **Forecasting (`05_benchmarking`) — the project's novel contribution. SCOPED (D-36, `notebooks/05_benchmarking/forecasting_scope.md`).** Tasks: (A) hourly {1,6,12,24,48}h + (B) daily-mean {1,3,7,14}d; **driver-conditional** (future drivers supplied); **train on gap-filled, evaluate on observed**; **leak-free** (FCO₂/GPP lagged-only — the gap-filling lever is gone → expect low R², lead with skill-vs-baseline). Test targets = **Towers 4 & 9** (T2 has no post-2019 CH₄ → pooled donor only). **First concrete step:** precompute the continuous gap-filled CH₄ series (`fch4_gapfilled.csv`, F-06/F-08 best gap-filler) + the forecasting feature matrix (AR lags + future exog + planned mgmt/livestock); then baselines (persistence/seasonal) → RF/XGB → LSTM/TFT, rolling-origin temporal CV (D-04/D-05).
   - ⚠ **Held-out 2024 still empty:** the hourly index now runs to 2025-01-02, but **2024 FCH₄ = 0% valid for all towers** — the final held-out benchmark is blocked until 2024 EC fluxes are downloaded. Test on 2022–2023 meanwhile.
