"""U-08 fan-chart plots: same visual convention as `u02_fanchart_plots.py`/`u04_fanchart_plots.py`,
applied to U-08's chains (`results/u08_chains.csv`/`u08_summary.csv`, the actual B18 champion --
direct TabPFN + p95 spike-gate, on `BASE_ALL_52`). Only one model this time, so 3 towers x 5
anchors = 15 figures.

Outputs to results/figures/u08_fancharts/T{tower}_anchor{year}_B18_TabPFN_champion.png.
"""
...
FIG_DIR = Path(RESULTS) / "figures" / "u08_fancharts"
...
def plot_fanchart(dft, chain_sub, margins_by_bin, tower, yr, anchor):
    ...
    ax.set_title(f"Tower {tower}, anchor {anchor.date()}, model={MODEL}, BASE_ALL_52 (U-08 fan chart)")
...
def main():
    ...
    summary = pd.read_csv(f"{RESULTS}/u08_summary.csv")
