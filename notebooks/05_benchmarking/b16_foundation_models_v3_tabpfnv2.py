"""D-8x: TabPFN v2-vs-v3 A/B. Same F-10 7-config x 3-tower x 5-anchor sweep as
`b16_foundation_models_v3.py`, but TabPFN is forced onto the OLD generic tabular v2 checkpoint
(`recursive_rollout.tabpfn_v2_model_config()`) instead of the TS-finetuned v3 checkpoint that
`tabpfn_forecast()` resolves to by default -- every existing TabPFN result in this project
(including the standing champion) already used v3, so this tests whether v3's TS-specific
finetuning actually helps versus the plain generic-tabular checkpoint the original TabPFN-TS
paper's approach was built on.

TabICLv2 is deliberately NOT included here (irrelevant to a TabPFN-version question; its results
already exist in `b16_foundation_models_v3_summary.csv` -- avoid an unnecessary rerun).

Run from project root:  python notebooks/05_benchmarking/b16_foundation_models_v3_tabpfnv2.py
"""
import os
import sys
import time
...
def main():
    ...
    if not tabpfn_ok:
        print("WARNING: TABPFN_TOKEN not set -- TabPFN_v2 will be skipped.")
            ...
            for cfg_name, fx_cols in configs.items():
                ...
                if tabpfn_ok:
                    try:
                        ...
                        bm["target"] = "observed"; bm["model"] = "TabPFN_v2"
                        bm["config"] = cfg_name
                        bm["anchor_year"] = yr
                        bm["tower"] = tower
                        all_rows.append(bm)
                        bm_gf = rr.bin_metrics(y_gf, yp, target_dates, anchor, y_persist=persist)
                        bm_gf["target"] = "gapfilled"; bm_gf["model"] = "TabPFN_v2"
                        bm_gf["config"] = cfg_name
                        bm_gf["anchor_year"] = yr
                        bm_gf["tower"] = tower
                        all_rows.append(bm_gf)
                    except Exception as e:
                        print(f"    T{tower} {yr} {cfg_name} TabPFN_v2 SKIPPED: {str(e)[:150]}")

            print(f"  Tower {tower} anchor {yr} done ({time.time()-t0:.0f}s elapsed)")

    out = pd.concat(all_rows, ignore_index=True)
    out.to_csv(f"{RESULTS}/b16_foundation_models_v3_tabpfnv2_summary.csv", index=False)
    print(f"\n[OK] Saved b16_foundation_models_v3_tabpfnv2_summary.csv ({len(out)} rows), total {time.time()-t0:.0f}s")
