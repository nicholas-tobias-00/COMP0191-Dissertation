"""D-79 follow-up combining two previously-separate lines of work that were never tested together:
F-10/D-67's 7-config feature-family sweep (b16_foundation_models_v3.py) and D-72's finding that
TabPFN/TabICLv2 genuinely improve when given `y_gapfilled` (not `y_observed`) as their historical
context. D-72 only tested the gap-filled-context choice on the plain BASE feature set
(`forecast_daily_v2.csv`); this combines it with the F-10 feature families on `forecast_daily_v3.csv`
-- the only code change from `b16_foundation_models_v3.py` is `hist_target = hist["y_gapfilled"]`
instead of `hist["y_observed"]` (mirrors `b10_b13_foundation_gf_extension.py`'s exact ablation).

Evaluation ground truth is untouched (real y_observed) -- only the historical context changes.

Run from project root:  python notebooks/05_benchmarking/b16_foundation_models_v3_gf.py
"""
import os
import sys
import time
...
def main():
    ...
    out.to_csv(f"{RESULTS}/b16_foundation_models_v3_gf_summary.csv", index=False)
    print(f"\n[OK] Saved b16_foundation_models_v3_gf_summary.csv ({len(out)} rows), total {time.time()-t0:.0f}s")
