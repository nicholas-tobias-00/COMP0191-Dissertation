"""D-79 follow-up to F-10/D-67's b16_foundation_models_v3.py: reruns the exact same TabPFN +
TabICLv2 sweep (all 7 feature configs x 3 towers x 5 anchors), but sourced from the TabICL-solo
gap-filled daily table (`forecast_daily_v3_tabicl.csv`, D-79) instead of the RFm champion's
(`forecast_daily_v3.csv`) -- tests whether the upstream gap-filling improvement (TabICL-solo beats
RFm at Tower 2/4) carries through to the downstream forecasting benchmark. Does not touch
b16_foundation_models_v3.py or its output; a parallel, additive comparison.

Note (checked before running this): the installed `tabpfn_time_series` (1.2.0) LOCAL-mode default
checkpoint is `tabpfn-v3-regressor-v3_20260506_timeseries.ckpt` -- a genuine TabPFN v3 checkpoint,
already the default `tabpfn_forecast()` (mode="local") resolves to with no code changes needed.
This means the original F-10/D-67 champion almost certainly already used TabPFN v3 -- this rerun
is a genuine "revised gap-filling data" test, not a "new TabPFN generation" test.

Run from project root:  python notebooks/05_benchmarking/b16_foundation_models_v3_tabicl.py
"""
import os
import sys
import time
...
def main():
    ...
    out.to_csv(f"{RESULTS}/b16_foundation_models_v3_tabicl_summary.csv", index=False)
    print(f"\n[OK] Saved b16_foundation_models_v3_tabicl_summary.csv ({len(out)} rows), total {time.time()-t0:.0f}s")
